...
def paginate_questions(request, selection):
  ...
  current_qustions = questions[start:end]
...
# create and configure the app
  ...
  def get_question_by_category(category_id):
    ...
    current_qustions = paginate_questions(request, question)
  ...
  def play_quizze():
    ...
    try:
      if quiz_category['id'] == 0 and len(previous_question) > 0:
        questions = Question.query.filter(Question.id.notin_(previous_question)).all()
      elif quiz_category['id'] != 0 and len(previous_question) > 0:
